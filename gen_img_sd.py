...
Usage
  python img_sd_webui_batch.py                       # uses defaults
  SD_API=http://127.0.0.1:7861 python img_sd_webui_batch.py  # override via env
  python img_sd_webui_batch.py --style "soft cinematic"
"""
...
OUTDIR_DEFAULT = Path(paths.OUTPUTS) / "images"
SD_API_DEFAULT = os.getenv("SD_API", "http://127.0.0.1:7860")

# --------------------------- Safety helpers ---------------------------------
# Negative prompt to keep it church‑safe and clean. Adjust as needed.
NEGATIVE_PROMPT = (
...
def sd_healthcheck(api_base: str, timeout: int = 10) -> None:
    """Ping a lightweight endpoint to fail fast if API is down."""
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # No --api flag; use env SD_API or default. Keep width/height/etc. overridable.
    ap.add_argument("--w", type=int, default=1024)
    ap.add_argument("--h", type=int, default=576)
    ap.add_argument("--steps", type=int, default=28)
    ap.add_argument("--cfg", type=float, default=5.5)
    ap.add_argument("--sampler", type=str, default="DPM++ 2M Karras") # try "DPM++ SDE Karras" in future
...
def main() -> None:
    ...
    api_base = SD_API_DEFAULT  # fixed here; override via SD_API env only
