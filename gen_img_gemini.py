#!/usr/bin/env python3
"""
Generate images for each clip in outputs/plan.json using Gemini 2.5 Flash Image ("Nano Banana").

- Reads per-clip `image_prompt` from the plan.
- Optional style suffix like image_gen_batch.py.
- Saves PNGs to outputs/images/clipN.png.
- Optional local daily hard cap.
...
Usage:
  python nano_banana_gemini.py
  python nano_banana_gemini.py --plan outputs/plan.json --outdir outputs/images
  python nano_banana_gemini.py --style "soft cinematic lighting, natural colors"
  python nano_banana_gemini.py --max-per-day 60
"""
...
DEFAULT_STATE = Path("outputs/gemini_state.json")
...
def main() -> None:
    ...
    if not os.getenv("GEMINI_API_KEY"):
        print("Missing GEMINI_API_KEY in environment or .env", file=sys.stderr)
        sys.exit(1)
